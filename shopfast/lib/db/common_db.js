...
// findOneByQuery: return a promise.
// resolve: get back a document by query.
function findOneByQuery(collection, query) {
  return new Promise((resolve, reject) => {
...
function updatePull(collection, query, body) {
  return new Promise((resolve, reject) => {
    collection.update(query, { $pull: body }, (error, result) => {
      if (error) {
        reject(error);
      }
      else {
...
function updateSetPush(collection, query, body) {
  if (_.isEmpty(query)) {
    throw errorUtil.makeError('Missing query', 500);
  }
